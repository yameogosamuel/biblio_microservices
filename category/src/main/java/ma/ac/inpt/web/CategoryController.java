...
public class CategoryController {
	...
	public List<Categorie> getCategoriesAndLivres(){
		...
		return cat.getListcat().stream()
                .map(c -> {
                     CatLivre catLiv = restTemplate.getForObject("http://localhost:8080/livre-service/livresByCat/" + c.getId(), CatLivre.class);
	...
	public Categorie getCategoryAndLivres(@PathVariable String category){
		...
		CatLivre catLiv = restTemplate.getForObject("http://localhost:8080/livre-service/livresByCat/" + cat.getId(), CatLivre.class);
